...
export const ReportPetEdit = () => {
  initialReport.id = usePetToEdit((state) => state.id);
  initialReport.photo = usePetToEdit((state) => state.photo);
  const navigate = useNavigate();
  const token = useAuthState((state) => state.token);
  const { updatePet, deletePet } = usePetToEdit();
  ...
  const handleDelete = async () => {
    deletePet(initialReport.id, initialReport.photo).then(() => {
      Swal.fire({
        icon: "success",
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await updatePet({ id: resReport.id, name, photo, lat, lng, location });
      Swal.fire({
        icon: "success",
